...
Qwen2.5-0.5B / 1.5B，WikiText-2 困惑度（2048-token 非重叠窗口，32 块 ≈ 65K tokens），
单卡 RTX A5000，bf16，全部由本仓库 CLI 产出（记录见 `results/*.json`）：
...
2. **分组粒度决定量化质量**：同为 W4，g128（15.69）比 per-channel（23.99）
  好 8.3 个 PPL——scale 的自由度换来的收益直观可见；W3 在 RTN 下开始崩坏（51.4）。
3. **模型越大越耐压**：50% Wanda 剪枝的代价，0.5B 是 +12.3 PPL，1.5B 只有
  +4.1；RTN W4 则是 +2.5 vs +1.1。压缩收益的规模效应在这张表里一目了然。
