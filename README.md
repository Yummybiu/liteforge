...
## V2.1 实测结果（2026-09-05）

| 实验 | 结果 | 说明 |
|---|---|---|
| SmoothQuant W8A8 α 扫描 | 最优 **α=0.7**（两模型一致），无平滑→最优 **3.65×/3.98×** | 蒙特卡洛损失口径；文献常用 0.5，搜索曲线本身就是答案 |
| 投机解码（1.5B←0.5B, k=4） | bf16: α=0.899, tok/fwd=4.57, 1.41×；**fp32: exact=True, 1.96×** | tok/fwd = α·k+1 与理论吻合；bf16 位级差异会翻转近平局 argmax（定理成立，浮点有现实） |
| 投机解码 k=8 | 加速 0.97×<1 | 无 KV cache 被每前向固定开销支配——诚实口径，不冒充部署数据 |
| MMLU-mini（dense 基线） | 0.5B=**47.6%** / 1.5B=**57.2%** | 似然打分口径；压缩模型的下游对照锚点 |
| **预算分配 @2.75bit** | DP: 68 层 W2+91 层 W3+9 层 W4（0.5B）| 精确 DP 逐层"剪/量化/保留"方案，分配地图见 results/ |
| **损失模型预测力** | **Spearman ρ=1.000**（0.5B n=6 / 1.5B n=6） | 排序完美迁移（跨方法族），幅度不迁移——误差复合机理见 decision_log D21 |
| 预算下限发现 | ≤2.75bit 时 RTN 族菜单全体崩溃（651~48万 PPL） | "分配救不了低于方法族地板的预算"；低比特菜单应换 GPTQ 口径 |
...
3. **量化分组粒度决定质量，误差反馈白赚一个点**：同为 W4，g128（15.69）比
   per-channel（23.99）好 8.3 个 PPL；自研 GPTQ 在 RTN 之上三点全线再赚
   （14.13/9.91/**8.34** vs 15.69/10.37/9.00）——3B 上 W4 距 fp16 无损仅 0.33。
