...
2. **误差补偿 + 块级动态重评分是剪枝的最优解**：自研 OBC 的演化三部曲——
  静态预选掩码崩塌（54.16，"互相保护的列簇被集体删除"），SparseGPT 忠实版
  块级动态重评分反超所有免训练基线（0.5B **17.52** vs Wanda 25.52；1.5B **11.67**
  vs 13.34）。高稀疏度下，删谁的决定必须跟着补偿动态走。
